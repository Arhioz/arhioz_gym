...
@suscripcion_router.post("/", response_model=schemas.SuscripcionResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(auth.RequiereRol(["administrador", "recepcion"]))])
...
@suscripcion_router.put("/{suscripcion_id}/cambiar_estado", response_model=schemas.SuscripcionResponse, dependencies=[Depends(auth.RequiereRol(["administrador", "recepcion"]))])
...
@suscripcion_router.put("/{suscripcion_id}/congelar", response_model=schemas.SuscripcionResponse, dependencies=[Depends(auth.RequiereRol(["administrador", "recepcion"]))])
...
@suscripcion_router.put("/{suscripcion_id}/reactivar", response_model=schemas.SuscripcionResponse, dependencies=[Depends(auth.RequiereRol(["administrador", "recepcion"]))])
    ...
    if getattr(usuario_actual, "tipo_usuario", None) == "cliente" or not hasattr(usuario_actual, "rol_id"):
        if usuario_actual.id != cliente_id:
            raise HTTPException(
                ...
                detail="No tienes permiso para consultar las suscripciones de otro cliente."
            )
    return await crud_suscripciones.obtener_suscripciones_por_cliente(db=db, cliente_id=cliente_id)

@suscripcion_router.delete("/{suscripcion_id}", status_code=status.HTTP_200_OK, dependencies=[Depends(auth.RequiereRol(["administrador", "recepcion"]))])
